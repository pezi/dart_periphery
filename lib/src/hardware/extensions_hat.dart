...
class HatCmd {
  ...
  /// Returns an command buffer containing only a command.
  List<int> getCmdSeq() {
...
const int I2C_READ_BUFFER_SIZE = 32;
...
class ArduinoBasedHat {
  ...
  List<int> read_i2c_block(int len) {
    ...
    for (var i = 0; i < RETRY; ++i) {
      try {
        var data = i2c.readBytesReg(HAT_ARDUINO_I2C_ADDRESS, 1, len);
        updateLastAction();
        return data;
  ...
  void write_i2c_block(List<int> data) {
    ...
    for (var i = 0; i < RETRY; ++i) {
      try {
        i2c.writeBytesReg(HAT_ARDUINO_I2C_ADDRESS, 1, data);
        updateLastAction();
        return;
      } on I2Cexception catch (e) {
        error = e;
  ...
  int analogRead(int pin) {
    ...
    for (var i = 0; i < RETRY; ++i) {
      try {
        ...
        sleep(Duration(milliseconds: DELAY));
        var data = i2c.readBytesReg(HAT_ARDUINO_I2C_ADDRESS, 1, 4);
  ...
  int readUltrasonic(int pin) {
    ...
    for (var i = 0; i < RETRY; ++i) {
      try {
        ...
        sleep(Duration(milliseconds: 100));
        var data = i2c.readBytesReg(HAT_ARDUINO_I2C_ADDRESS, 1, 3);
        updateLastAction();
...
class NanoHatHub extends ArduinoBasedHat {
  ...
  void servoWrite(int pin, int value) {
    _sendCmd(SERVO_WRITE, pin, value);
  }
}

enum Color { GREEN, RED, YELLOW, BLUE, GHOST_WHITE, ORANGE, CYAN }

enum Led { LED1, LED2, LED3, LED4, LED5 }
